...
def validorder(order: Order):
    ...
    for item in order.items:
        if item.type == 'payment':
            ...
            if isinstance(item.quantity, int) and MIN_QUANTITY <= item.quantity <= MAX_QUANTITY and MIN_QUANTITY <= item.amount <= MAX_AMOUNT:
                expenses += Decimal(str(item.amount)) * item.quantity
        else:
            return "Invalid item type: %s" % item.type

    if abs(payments) > MAX_AMOUNT or abs(expenses) > MAX_AMOUNT:
